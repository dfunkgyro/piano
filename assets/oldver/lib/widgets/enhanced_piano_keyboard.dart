...
class KeyboardSettings {
  ...
  KeyboardSettings({
    ...
    this.keyWidthScale = 1.0,
    this.blackKeyWidthFactor = 0.6,
    this.blackKeyHeightFactor = 0.6,
...
class EnhancedPianoKeyboard extends StatefulWidget {
  ...
  final Set<int> guideNotes;
  final Map<int, String> guideHands;
  final int? minNote;
  final int? maxNote;
  ...
  const EnhancedPianoKeyboard({
    ...
    this.wrongNotes,
    this.heatmap = const {},
    this.guideNotes = const {},
    this.guideHands = const {},
    this.minNote,
    this.maxNote,
    this.onKeyPressed,
    this.onKeyReleased,
...
class _EnhancedPianoKeyboardState extends State<EnhancedPianoKeyboard>
  ...
  int _rangeStart = startNote;
  int _rangeEnd = endNote;
  ...
  void _updateRange() {
    _rangeStart = widget.minNote ?? startNote;
    _rangeEnd = widget.maxNote ?? endNote;
    if (_rangeStart < startNote) _rangeStart = startNote;
    if (_rangeEnd > endNote) _rangeEnd = endNote;
    if (_rangeEnd <= _rangeStart) {
      _rangeStart = startNote;
      _rangeEnd = endNote;
    }
  }
  ...
  void _buildNoteLists() {
    ...
    int whiteIndex = 0;
    for (int note = _rangeStart; note <= _rangeEnd; note++) {
      if (_isBlackKey(note)) {
  ...
  void didUpdateWidget(EnhancedPianoKeyboard oldWidget) {
    ...
    if (widget.minNote != oldWidget.minNote ||
        widget.maxNote != oldWidget.maxNote) {
      _updateRange();
      _buildNoteLists();
    }
  ...
  double _getBlackKeyPosition(int midiNote, double whiteKeyWidth) {
    ...
    int whiteKeyIndex = 0;
    for (int i = _rangeStart; i < midiNote; i++) {
      if (!_isBlackKey(i)) {
        whiteKeyIndex++;
      }
    }
    ...
    final blackKeyWidth = whiteKeyWidth * widget.settings.blackKeyWidthFactor;
    double offset = whiteKeyWidth - (blackKeyWidth / 2);
  ...
  Color _getKeyColor(int midiNote, bool isPressed) {
    ...
    if (!isPressed) {
      final base = isBlack ? const Color(0xFF1A1A1A) : Colors.white;
      return _applyHeatmap(base, midiNote, isBlack);
    }
    ...
    return _applyHeatmap(pressedColor, midiNote, isBlack);
  }

  Color _applyHeatmap(Color base, int midiNote, bool isBlack) {
    final intensity = widget.heatmap[midiNote] ?? 0.0;
    if (intensity <= 0.01) return base;
    final target = isBlack ? Colors.orangeAccent : Colors.deepOrange;
    return Color.lerp(base, target, intensity.clamp(0.0, 1.0) * 0.6)!;
  }
  ...
  Widget _buildKeyboard() {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final keyboardWidth =
            constraints.maxWidth * widget.settings.keyWidthScale;
        final effectiveWidth =
            math.max(constraints.maxWidth, keyboardWidth);

        final whiteKeyWidth = effectiveWidth / whiteKeyCount;
        final blackKeyWidth =
            whiteKeyWidth * widget.settings.blackKeyWidthFactor;
        final blackKeyHeight =
            constraints.maxHeight * widget.settings.blackKeyHeightFactor;
        ...
        if (effectiveWidth <= constraints.maxWidth) {
          return keyboard;
        }
    ...
    return SizedBox(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          final keyboard = RepaintBoundary(
            child: Listener(
              onPointerDown: (event) => _handlePointerDown(
                  event.pointer,
                  event.localPosition,
                  Size(effectiveWidth, constraints.maxHeight)),
              ...
              child: CustomPaint(
                size: Size(effectiveWidth, constraints.maxHeight),
                painter: _PianoKeyboardPainter(
                  ...
                  guideNotes: widget.guideNotes,
                  guideHands: widget.guideHands,
                ),
              ),
            ),
          );
          ...
          return SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: SizedBox(width: effectiveWidth, child: keyboard),
          );
        },
      ),
    );
  ...
  int? _hitTestNote(Offset position, Size size) {
    ...
    final blackKeyWidth =
        whiteKeyWidth * widget.settings.blackKeyWidthFactor;
    final blackKeyHeight =
        size.height * widget.settings.blackKeyHeightFactor;
  ...
  Widget _buildKeyContent(int midiNote, bool isBlack, bool isPressed) {
    if (!widget.settings.showNoteNames &&
        !widget.guideNotes.contains(midiNote)) {
      return const SizedBox.shrink();
    }

    final guide = widget.guideHands[midiNote];
...
class _PianoKeyboardPainter extends CustomPainter {
  ...
  _PianoKeyboardPainter({
    ...
    required this.guideHands,
  });

  Color _applyHeatmap(Color base, int midiNote, bool isBlack) {
    final intensity = heatmap[midiNote] ?? 0.0;
    if (intensity <= 0.01) return base;
    final target = isBlack ? Colors.orangeAccent : Colors.deepOrange;
    return Color.lerp(base, target, intensity.clamp(0.0, 1.0) * 0.6)!;
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final blackKeyWidth = whiteKeyWidth * settings.blackKeyWidthFactor;
    final blackKeyHeight = size.height * settings.blackKeyHeightFactor;
    ...
    for (int i = 0; i < whiteNotes.length; i++) {
      ...
      final color = _applyHeatmap(getKeyColor(note, isPressed), note, false);
    ...
    for (final note in blackNotes) {
      ...
      final isPressed = activeNotes.contains(note);
      final color = _applyHeatmap(getKeyColor(note, isPressed), note, true);
    ...
    for (final note in guideNotes) {
      final isBlack = blackNotes.contains(note);
      double left;
      double top;
      double width;
      if (isBlack) {
        final leftWhite = blackLeftWhiteIndex[note];
        if (leftWhite == null || leftWhite < 0) continue;
        left = (leftWhite + 1) * whiteKeyWidth - (blackKeyWidth / 2);
        top = blackKeyHeight - 24;
        width = blackKeyWidth;
      } else {
        final whiteIndex = whiteNotes.indexOf(note);
        if (whiteIndex < 0) continue;
        left = whiteIndex * whiteKeyWidth + (keySpacing / 2);
        top = size.height - 28;
        width = whiteKeyWidth - keySpacing;
      }
      final hand = guideHands[note] ?? '';
      if (hand.isEmpty) continue;
      final bgColor = hand == 'L'
          ? Colors.blue.withOpacity(0.8)
          : Colors.red.withOpacity(0.8);
      final rect = RRect.fromRectAndRadius(
        Rect.fromLTWH(left + width / 2 - 10, top, 20, 14),
      ...
      textPainter.paint(
        canvas,
        Offset(left + width / 2 - textPainter.width / 2, top - 1),
      );
    }
  }
  ...
  bool shouldRepaint(covariant _PianoKeyboardPainter oldDelegate) {
    return oldDelegate.activeNotes != activeNotes ||
        ...
        oldDelegate.guideHands != guideHands;
  }
}
