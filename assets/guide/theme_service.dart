...
class ThemeService {
  ...
  static const Map<AppLayout, LayoutData> layouts = {
    ...
    AppLayout.professional: LayoutData(
      name: 'Professional',
      description: 'Studio-grade interface with large keyboard and AI coaching',
      keyboardHeight: 260, // Larger keyboard for professional use
      showQuickStats: true,
      showAIBanner: true,
      compactControls: false,
      padding: EdgeInsets.all(16),
  ...
  // Default to dark theme and professional layout
  static AppTheme _currentTheme = AppTheme.dark;
  static AppLayout _currentLayout = AppLayout.professional;

  static Future<void> loadTheme() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final themeIndex = prefs.getInt(_themeKey);
      final layoutIndex = prefs.getInt(_layoutKey);

      // If saved preferences exist, use them; otherwise keep defaults
      if (themeIndex != null && themeIndex < AppTheme.values.length) {
        _currentTheme = AppTheme.values[themeIndex];
      }
      if (layoutIndex != null && layoutIndex < AppLayout.values.length) {
        _currentLayout = AppLayout.values[layoutIndex];
      }
    } catch (e) {
      print('Error loading theme: $e, using defaults (dark theme, professional layout)');
    }
  }
