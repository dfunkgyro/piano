...
import 'score_complete_song_lesson_screen.dart';
import 'score_lesson_screen.dart';

class SafeScoreLibraryScreen extends StatefulWidget {
  const SafeScoreLibraryScreen({super.key});

  @override
  State<SafeScoreLibraryScreen> createState() =>
      _SafeScoreLibraryScreenState();
}

class _SafeScoreLibraryScreenState extends State<SafeScoreLibraryScreen> {
                    ...
                    children: [
                      ...
                      if (_collection != 'Full')
                        _section(
                          theme,
                          'Quick Studies',
                          'Short score studies and guided reading drills',
                      ...
                      if (_collection != 'Quick')
                        _section(
                          theme,
                          'Full Scores',
                          'Full sheet music arrangements and complete studies',
                          '${fullFiltered.length}',
                        ),
                      if (_collection != 'Quick')
                        ..._buildFullResults(
    ...
    return MotionCard(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'A cleaner score browser with search, filtering, and simpler paths into notation practice.',
            style: TextStyle(
  ...
  List<Widget> _buildQuickResults(
    ...
    if (songs.isEmpty) {
      return [_emptyState(theme, 'No score quick studies match these filters.')];
    }
  ...
  List<Widget> _buildFullResults(
    ...
    if (songs.isEmpty) {
      return [_emptyState(theme, 'No full score pieces match these filters.')];
    }
    return songs.asMap().entries.map((entry) {
      return MotionReveal(
        delay: Duration(milliseconds: 40 + (entry.key * 18)),
              ...
              CupertinoButton.filled(
                ...
                onPressed: () async {
                  ...
                  Navigator.of(context).push(
                    CupertinoPageRoute(builder: (_) => ScoreLessonScreen(song: song)),
                  );
                },
            ...
            children: [
              ...
              CupertinoButton.filled(
                ...
                onPressed: () async {
                  ...
                  Navigator.of(context).push(
                    CupertinoPageRoute(
                      builder: (_) => ScoreCompleteSongLessonScreen(song: song),
                    ),
                  );
                },
                child: const Text('Open'),
