...
class ConnectionManagerService {
  ...
  static Future<void> clearAllDevices() async {
    try {
      _savedDevices.clear();
      await _saveSavedDevices();
      debugPrint('??? Cleared all saved devices');
    } catch (e) {
  ...
  Future<bool> connectToMidiDevice(
    ...
    try {
      ...
      if (bleDevice != null) {
        ...
        debugPrint('?? BLE handshake failed, but continuing to stage 2');
  ...
  Future<fbp.BluetoothDevice?> _performBleHandshake(
    ...
    try {
      ...
      final bleDevice = await _findBleDevice(deviceName, deviceId: deviceId);

      if (bleDevice == null) {
        debugPrint('?? BLE device not found in scan');
        if (deviceId != null) {
          final ok = await _universalBleHandshake(deviceId);
          if (ok) {
            return null;
          }
        }
      ...
      try {
        ...
        debugPrint('?? BLE handshake failed, trying universal_ble: $e');
        if (deviceId != null) {
          await _universalBleHandshake(deviceId);
        }
      }

      return bleDevice;
    } catch (e) {
      debugPrint('?? BLE handshake error (non-fatal): $e');
      return null;
    }
  }

  /// Find BLE device by name
  Future<fbp.BluetoothDevice?> _findBleDevice(
    String targetName, {
    String? deviceId,
  }) async {
    ...
    try {
      final subscription = fbp.FlutterBluePlus.scanResults.listen((results) {
        for (var result in results) {
          ...
          if (deviceId != null && device.remoteId.str == deviceId) {
          ...
          if (hasMidiService &&
              (targetName.toLowerCase().contains('widi') ||
                  targetName.toLowerCase().contains('midi'))) {
            if (!completer.isCompleted) {
              completer.complete(device);
            }
            continue;
  ...
  Future<bool> _patientMidiConnect(
    ...
    final extendedTimeout =
        nameLower.contains('widi') || nameLower.contains('midi');
    final timeout = extendedTimeout
        ? const Duration(seconds: 15)
        : osRecognitionTimeout;

    final maxAttempts = (timeout.inMilliseconds /
    ...
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        if (devices == null || devices.isEmpty) {
          ...
          if (!nameOnly && deviceId.isNotEmpty) {
            try {
              ...
              targetDevice = devices.firstWhere(
                (d) {
                  ...
                  return dName == nameLower ||
                      dName.contains(nameLower) ||
                      nameLower.contains(dName) ||
                      _normalizeName(dName)
                          .contains(_normalizeName(nameLower));
                },
              );
              debugPrint('? Found by name: ${targetDevice.name}');
            } catch (_) {
              debugPrint('?? Device not in OS MIDI list yet');
            }
          }
  ...
  Future<bool> _universalBleHandshake(String deviceId) async {
    try {
      await ub.UniversalBle.connect(deviceId);
      await Future.delayed(const Duration(milliseconds: 600));
      await ub.UniversalBle.disconnect(deviceId);
      return true;
    } catch (e) {
      debugPrint('?? Universal BLE handshake failed: $e');
      return false;
    }
  }
