...
class MidiService {
  ...
  Future<void> _initialize() async {
    ...
    try {
      ...
      if (kIsWeb) {
        _isInitialized = true;
        _log('?? Web platform: skipping MIDI native init');
        return;
      }
  ...
  Future<void> _requestPermissions() async {
    if (kIsWeb) {
      _log('?? Web platform: skipping Bluetooth permission checks');
      return;
    }
    if (defaultTargetPlatform == TargetPlatform.macOS) {
        ...
        for (var result in results) {
          ...
          if (name.isEmpty) {
            final hasMidiService = result.advertisementData.serviceUuids
                .any((uuid) =>
                    uuid.toString().toLowerCase() == '03b80e5a-ede8-4b33-a751-6ce34ec4c700');
            if (hasMidiService) {
  ...
  Future<void> _scanUniversalBleFallback() async {
