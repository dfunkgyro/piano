...
class KeyboardSettingsPanel extends StatefulWidget {
  ...
  final String? ultraMode;
  final Function(String)? onUltraModeChanged;
  final bool? qwertyEnabled;
  final Function(bool)? onQwertyChanged;
...
class _KeyboardSettingsPanelState extends State<KeyboardSettingsPanel> {
  ...
  void initState() {
    ...
    _volume = widget.masterVolume ?? 0.8;
    _ultraMode = widget.ultraMode ?? 'off';
    _qwertyEnabled = widget.qwertyEnabled ?? true;
  }
  ...
  Widget _buildAudioSettings() {
    return Column(
      ...
      children: [
        ...
        Text(
          'Boost above 100% for louder output',
          style: const TextStyle(
            fontSize: 12,
  ...
  Widget _buildAdvancedSettings() {
      ...
      children: [
        ...
        const SizedBox(height: 16),
          _buildToggle(
            ...
            (value) {
              _updateSettings(_settings..performanceMode = value);
            },
            subtitle: 'Lowest latency, preload audio, fewer visual effects',
          ),
          if (widget.onUltraModeChanged != null) ...[
            const SizedBox(height: 16),
            _buildUltraModeSelector(),
          ],
          if (widget.onQwertyChanged != null) ...[
            const SizedBox(height: 16),
            _buildToggle(
              ...
              (value) {
                ...
                widget.onQwertyChanged?.call(value);
              },
              subtitle: 'Use computer keyboard to play notes (Z/X octave)',
            ),
          ],
        ],
      );
    ...
    Widget _buildUltraModeSelector() {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text(
          ...
          CupertinoSegmentedControl<String>(
            ...
            onValueChanged: (value) {
              ...
              widget.onUltraModeChanged?.call(value);
            },
          ),
          const SizedBox(height: 6),
          Text(
            _ultraMode == 'audioOnly'
                ? 'Preload all notes and prioritize audio latency.'
                : _ultraMode == 'polyphony'
                    ? 'Max voices for dense chords (higher CPU/memory).'
                    : _ultraMode == 'visuals'
                        ? 'Reduce visuals for smoother UI while playing.'
                        : 'Off by default.',
            style: const TextStyle(
              fontSize: 12,
              color: CupertinoColors.systemGrey,
